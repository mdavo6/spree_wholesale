Spree.ready ($) ->
  Spree.onAddress = () ->
    if ($ '.wholesaler-form, .new_spree_wholesaler').is('*')
      ...
      wholesaler_use_billing = ($ 'input#wholesaler_use_billing')
      wholesaler_use_billing.change ->
        update_shipping_form_state wholesaler_use_billing

      update_shipping_form_state = (wholesaler_use_billing) ->
        if wholesaler_use_billing.is(':checked')
          ($ '#shipping .inner').hide()
